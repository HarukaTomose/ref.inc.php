...
// branch: 2021/10/16 H.Tomose
// pukiwiki orizinal ref v1.51 を基準に改造。
//
// 変更目的
// サムネイルで画像表示→クリックで元表示動作。
// ・独自にサムネイルを作ることはしない。
// ・上記も含めて、サムネイル画像は独自に添付する必要あり。
// 
// usage
// &ref(image,....<,thumb=xxx>)
// 任意位置に「thumb」というパラメータがある場合、その引数をサムネイルファイルとして取り出す。
...
function plugin_ref_body($args)
{
	...
	// thumb対応：まずパラメータで「サムネイル指定」があるかをチェック。
    foreach ( $args as $opt ){
		if(substr($opt,0,6) == 'thumb='){

		$name_thumb_ = explode('=', $opt);
		$name_thumb = $name_thumb_[1];
		$is_url = is_url($name_thumb);
		if( $is_url) {
			$params['_error'] = 'Thumbnail is MUST file.';
			return $params;
		}else {
			$matches = array();
			// ファイル名にページ名(ページ参照パス)が合成されているか
			//   (Page_name/maybe-separated-with/slashes/ATTACHED_FILENAME)
			if (preg_match('#^(.+)/([^/]+)$#', $name_thumb, $matches)) {
				if ($matches[1] == '.' || $matches[1] == '..') {
					$matches[1] .= '/'; // Restore relative paths
				}
				$name_thumb = $matches[2];
				$page = get_fullname(strip_bracket($matches[1]), $page); // strip is a compat
				$file_thumb = UPLOAD_DIR . encode($page) . '_' . encode($name_thumb);
				$is_file = is_file($file_thumb);
			} else {
			...
			if (! $is_file) {
				$params['_error'] = htmlsc('ThumbnailFile not found: "' .
					$name_thumb . '" at page "' . $page . '"');
				return $params;
			}
		}
	...
	// 適切なサムネイルファイルが添付されていれば、$name_thumb にサムネイル場所が入っている。

	// 以降、原則元プラグイン ref と全く同じ。
	// 元refの第一引数: "[ページ名および/]添付ファイル名"、あるいは"URL"を取得
	...
	// サムネイル指定がない場合、サムネイルにあたる画像も本画像を利用する。
	if($name_thumb==""){
		$name_thumb= $name;
		$file_thumb= $file;
	}
		...
		if ($is_image) {
			...
			if (! PLUGIN_REF_DIRECT_ACCESS) {
				...
				$url = $script . '?plugin=ref' . '&amp;page=' . rawurlencode($page) .
					'&amp;src=' . rawurlencode($name_thumb); // Show its filename at the last
			} else {
			...
			$width = $height = 0;
			$size = @getimagesize($file_thumb); // サムネイルのサイズで考える。
			if (is_array($size)) {
				$width  = $size[0];
				$height = $size[1];
			}
